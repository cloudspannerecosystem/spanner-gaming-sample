...
for generator in profile matchmaking game tradepost; do
    generator="${generator}-workload"
    echo "[INFO] Configuring ${generator}"
    sed "s/GCP_PROJECT_ID/${PROJECT_ID}/" "${generator}.yaml.tmpl" > "${generator}.yaml"

    echo "[INFO] Deploying ${generator}"
